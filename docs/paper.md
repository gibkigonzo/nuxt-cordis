...
|3.2.1. Coefect Context . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 18|
|3.2.2. Specifcation and Notifcation . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 19|
|3.2.3. Isolation and Interception . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 20|
|3.3. The Context Paradigm . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 22|
|3.3.1. Unifed Context . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 22|
...
|5.2.1. Declarative Confguration . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 62|
...
**Theorem 5.** trackΓ is a monoid homomorphism from 𝔗Γ into 𝜕Γ →𝜕Γ. That is, 1. trackΓ(idΓ, idΓ) = id𝜕Γ; 
...
𝑓<br>Γ Γ<br>𝑔<br>𝑒<br>effect pr2 pr1<br>𝑓 ′<br>𝜕Γ 𝜕Γ<br>𝑔 ′<br>𝑒 ′<br>pr2 pr1<br>𝜕 2 Γ<br><!-- End of picture text -->
...
1. By Definition 12, 𝑓<sup>′</sup> (𝛾, 𝜑) = (𝑓(𝛾), 𝜑∘𝑔), whose state is 𝑓(𝛾) = (𝑓∘pr1)(𝛾, 𝜑). 

2. This is Theorem 4 applied to 𝑔<sup>′</sup> = trackΓ(𝑔, 𝑓). □ 
...
1. 𝛿𝑢 = 𝑓𝑗(𝛿𝑢<sup>′) and 𝑔</sup> 𝑗<sup>(𝛿</sup> 𝑢<sup>) = 𝛿</sup> 𝑢<sup>′;</sup> 2. each 𝑒𝑖 with 𝑖> 𝑗 yields at 𝛿𝑖−1<sup>′the same inverse 𝑔</sup> 𝑖<sup>it yields at 𝛿</sup> 𝑖−1<sup>.</sup> 
...
𝑔𝑗(𝛿𝑢) = (𝑔𝑗 ∘𝑓𝑢 ∘⋯∘𝑓𝑗+1)(𝛿𝑗) = (𝑓𝑢 ∘⋯∘𝑓𝑗+1)(𝑔𝑗(𝑓𝑗(𝛿𝑗−1))) = 𝛿<sup>′</sup> 

𝑢 

the last equality resting on 𝑔𝑗(𝑓𝑗(𝛿𝑗−1)) = 𝛿𝑗−1, which is the witness Definition 8 requires of 𝑒 at 𝛿 . 𝑗 𝑗−1 
...
get : (𝑘: 𝐾) →Σ ⇀ 𝒱︀𝑘 get = 𝑘 ↦ 𝜎 ↦ 𝜎(𝑘) (21) set : (𝑘: 𝐾) × 𝒱︀𝑘 →Σ ⇀ Σ × (Σ ⇀Σ) set = (𝑘, 𝑣) ↦ 𝜎 ↦(𝜎[𝑘↦𝑣], 𝜆𝜎<sup>′</sup> .𝜎<sup>′</sup> ∖𝑘) 
...
|get|:|(𝑘: 𝐾)|→|Σ<sup>iso</sup>|⇀<br>𝒱𝜌(𝑘)|
|---|---|---|---|---|---|
|get|=|𝑘|↦|(𝜌, 𝜎)|↦<br>𝜎(𝜌(𝑘))|
|set|:|(𝑘: 𝐾) × 𝒱𝜌(𝑘)|→|Σ<sup>iso</sup>|⇀<br>Σ<sup>iso </sup>× (Σ<sup>iso </sup>⇀Σ<sup>iso</sup>)<br>(28)|
|set|=|(𝑘, 𝑣)|↦|(𝜌, 𝜎)|↦((𝜌, 𝜎[𝜌(𝑘) ↦𝑣]), 𝜆(𝜌<sup>′</sup>, 𝜎<sup>′</sup>).(𝜌<sup>′</sup>, 𝜎<sup>′ </sup>∖𝜌<sup>′</sup>(𝑘)))<br>|
|isolate|:|𝐾× 𝑅|→|Σ<sup>iso</sup>|→<br>Σ<sup>iso</sup>|
|isolate|=|(𝑘, 𝑟)|↦|(𝜌, 𝜎)|↦<br>(𝜌[𝑘↦𝑟], 𝜎)|
...
|get|:|(𝑘: 𝐾)|× ℳ𝑘<br>→|Σ<sup>inter</sup>|⇀<br>𝒱𝑘|
|---|---|---|---|---|---|
|get|=|(𝑘,|𝜇)<br>↦|(𝜄, 𝜎)|↦<br>𝜎(𝑘)(𝜇⊕𝑘𝜄(𝑘))|
|set|:|(𝑘: 𝐾) × (|ℳ𝑘→𝒱𝑘) →|Σ<sup>inter</sup>|⇀<br>Σ<sup>inter </sup>× (Σ<sup>inter </sup>⇀Σ<sup>inter</sup>)<br>(30)|
|set|=|(𝑘,|𝜓)<br>↦|(𝜄, 𝜎)|↦((𝜄, 𝜎[𝑘↦𝜓]), 𝜆(𝜄<sup>′</sup>, 𝜎<sup>′</sup>).(𝜄<sup>′</sup>, 𝜎<sup>′ </sup>∖𝑘))<br>|
|intercept|:|(𝑘: 𝐾)|× ℳ𝑘<br>→|Σ<sup>inter</sup>|→<br>Σ<sup>inter</sup>|
|intercept|=|(𝑘,|𝜈)<br>↦|(𝜄, 𝜎)|↦<br>(𝜄[𝑘↦𝜄(𝑘) ⊕𝑘𝜈], 𝜎)|
...
Calling the relation observational is a claim about each ≃𝑘<sup>, namely that it separates no more</sup> than the operations of 𝑘 can tell apart. An observer of a value runs those operations and reads their outcomes. 
...
**Lemma 35.** Indistinguishability is the coarsest relation the operations respect. That is, 1. every operation of 𝒜︀ respects ≈ 𝒜︀<sup>in the sense of Definition 24;</sup> 
...
1. Let 𝑣≈ 𝒜︀<sup>𝑣′ and let 𝑎∈𝒜︀ be applied to an argument. Prefixing a test by one letter is again</sup> 

a test, so the values the forward map reaches are indistinguishable, as are the values any one yielded inverse reaches from indistinguishable arguments; the one-letter test gives definedness at both or neither and equality of the outcome. 
...
2. 𝑔 respects ≃. Taking ≃ to be equality on Γ recovers Definition 8. 

**Lemma 38.** With 𝔈Γ<sup>∗read as in Definition 37, every equality of states asserted in Section 3.1 holds</sup> with = replaced by ≃, and the accumulator of every state reachable from (𝛾0, idΓ) respects ≃. _Proof._ An accumulator is a composition of inverses, each respecting ≃ by Definition 37(2), and a composition of maps respecting ≃ respects ≃, the base case being idΓ. The proofs of Section 3.1 then go through unchanged, respect being what carries a relation through an inverse: from 𝑔2(𝛿2) ≃𝛿1 and 𝑔1(𝛿1) ≃𝛾 respect gives (𝑔1 ∘𝑔2)(𝛿2) ≃𝛾, which is the step each composition of inverses takes, and the soundness invariant of Theorem 7 reads 𝜑(𝛾) ≃𝛾0 by that step. □ 
...
|**rule**|**𝜃**<sup>**𝑡**</sup><br>**𝑛**|**𝜃**<sup>**𝑡+1**</sup><br>**𝑛**|**Ψ**<sup>**𝑡**</sup>|**control felds edited**|
|---|---|---|---|---|
|O-Insert|undefned|𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(⊥)|idΓ|dom(𝐹𝛾)|
|O-Retire|unconstrained|unchanged|idΓ|𝜏𝑛|
|O-Remove|𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(−)|undefned|idΓ|dom(𝐹𝛾)|
...
1. 𝜎𝑚<sup>𝑡+1</sup> ≠𝜎𝑚<sup>𝑡only where step 𝑡 acts on 𝑚, the write lying inside Ψ𝑡;</sup> 2. 𝜔𝑛 comes into existence only where step<sup>𝑡</sup> = L-Begin(𝑛) and ceases only where step<sup>𝑡</sup> = L-Unload(𝑛), so 𝜔𝑛<sup>𝑡is constant for 𝑡 in an episode of 𝑛;</sup> 
...
- (1) By Table 1 only O-Insert and O-Remove write a 𝜋 or dom(𝐹𝛾). O-Insert has 𝜋𝑛 ∈dom(𝐹<sup>𝑡</sup> ) ∪ 
...
(3) By Lemma 54(2) the only rule that writes an 𝜔𝑛 is L-Begin, whose premise 𝜔= target<sup>𝑡</sup> 𝑛<sup>≠⊥</sup> makes it total on 𝑑𝑛 and valued in dom(𝐹<sup>𝑡</sup> ), target naming providers. By Table 1 the only rule that shrinks dom(𝐹𝛾) is O-Remove, whose premise 𝜃𝑛<sup>𝑡= 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(−) gives ¬ installed𝑡</sup> 𝑛<sup>, whence</sup> by clause (4) at 𝛾<sup>𝑡</sup> no 𝑚 has 𝜔𝑚<sup>𝑡(𝑘) = 𝑛 for a 𝑘∈𝑑</sup> 𝑚<sup>while installed𝑡</sup> 𝑚<sup>; and 𝑛 itself carries no 𝜔.</sup> 

(4) By Lemma 54(2) and (4) the clause can fail at 𝛾<sup>𝑡+1</sup> only where some installed has fallen, some 𝜔 has been written, or a fiber some 𝜔 names has left dom(𝐹𝛾). The last is an O-Remove, whose removed fiber is not installed and hence, by clause (4) at 𝛾<sup>𝑡</sup> , is named by no 𝜔𝑚<sup>𝑡of an</sup> installed 𝑚. The first is an L-Unload of 𝑛, whose premise ¬ relied<sup>𝑡</sup> 𝑛<sup>reads</sup> 
...
step<sup>𝑡</sup> = L-Begin(𝑚) ⇒𝛾<sup>𝑡</sup> ⊧𝑑𝑚 
...
2. 𝑏< 𝑏<sup>′</sup> , and 𝑢<sup>′</sup> < 𝑢 if [𝑏, 𝑢] closes; 

3. 𝑘∈dom(𝜎𝑛<sup>𝑡) and 𝜎</sup> 𝑛<sup>𝑡(𝑘) = 𝜎</sup> 𝑛<sup>𝑏′(𝑘).</sup> 

_Proof._ The first claim is the premise target<sup>𝑡</sup> 𝑚<sup>≠⊥ of L-Begin, which by Definition 46 gives 𝛾𝑡⊧</sup> 𝑑𝑚. 

#### (1) is Lemma 54(2). 

For (2), the L-Begin at 𝑏<sup>′</sup> −1 writes 𝜔𝑚<sup>𝑏′= target𝑏</sup> 𝑚<sup>′−1</sup> , whose values are providers, so 𝜃𝑛<sup>𝑏′=</sup> 𝖠𝖼𝗍𝗂𝗏𝖾(−, −); the L-Begin at 𝑏−1 leaves 𝜃𝑛<sup>𝑏= 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀(−, −, −), so𝑏≠𝑏′ and hence𝑏< 𝑏′,</sup> both episodes opening by Definition 53. Let [𝑏, 𝑢] close and suppose 𝑢≤𝑢<sup>′</sup> . Then 𝑢∈[𝑏<sup>′</sup> , 𝑢<sup>′</sup> ], so installed<sup>𝑢</sup> 𝑚<sup>and, by (1), 𝜔</sup> 𝑚<sup>𝑢(𝑘) = 𝑛; that is relied𝑢</sup> 𝑛<sup>, which the L-Unload at 𝑢 denies. Hence</sup> 𝑢<sup>′</sup> < 𝑢. 

For (3), 𝑛 is the provider of 𝑘 at 𝛾<sup>𝑏′</sup> , so 𝑘∈dom(𝜎𝑛<sup>𝑏′). No L-Unload of 𝑛 falls in [𝑏′, 𝑢′]: where</sup> [𝑏, 𝑢] closes it falls at 𝑢> 𝑢<sup>′</sup> by (2), and where it does not, Lemma 54(4) leaves 𝑛 with no L- Unload at all. Since 𝜃𝑛<sup>𝑏′= 𝖠𝖼𝗍𝗂𝗏𝖾(−, −), Table 1 therefore leaves L-Leave as the only rule 𝑛 can</sup> be acted on by within [𝑏<sup>′</sup> , 𝑢<sup>′</sup> ], and its Ψ<sup>𝑡</sup> is idΓ; by Lemma 54(1) 𝜎𝑛 is constant there. □ 
...
2. _(Termination.)_ 𝑆(𝑛) ≤(𝐾+ 4)(𝑉(𝑛) + 1), and both 𝑉(𝑛) and ∑𝑛 𝑆(𝑛) are finite. 
...
• 𝜃𝑛<sup>𝑡= 𝖠𝖼𝗍𝗂𝗏𝖾(−, 𝜔</sup> 𝑛<sup>) with target𝑡</sup> 𝑛<sup>≠𝜔</sup> 𝑛<sup>: L-Leave applies.</sup> Let no fiber be of any of these kinds, leaving some 𝑚0 with 𝜃𝑚<sup>𝑡</sup> 0<sup>= 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀(−, −, −). Construct</sup> 𝑚0, 𝑚1, … as follows: given 𝑚𝑗 in 𝖴𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀, either ¬ relied<sup>𝑡</sup> 𝑚𝑗<sup>, in which case L-Unload applies</sup> to 𝑚𝑗 and the construction stops, or there are 𝑚𝑗+1 ≠𝑚𝑗 and 𝑘𝑗 with installed<sup>𝑡</sup> 𝑚𝑗+1<sup>and</sup> 𝜔𝑚<sup>𝑡</sup> 𝑗+1<sup>(𝑘</sup> 𝑗<sup>) = 𝑚</sup> 𝑗<sup>. In the latter case</sup> 
...
(A) Over a maximal interval on which target<sup>𝑡</sup> 𝑛<sup>is constant at 𝜔∗, at most 𝐾+ 4 steps act on 𝑛.</sup> Reading the 𝜃𝑛 columns of Table 1, from 𝖠𝖼𝗍𝗂𝗏𝖾(−, 𝜔) with 𝜔≠𝜔<sup>∗</sup> the fiber takes an L-Leave and an L-Unload and then, if 𝜔<sup>∗</sup> ≠⊥, an L-Begin and at most len(𝑒𝑛) ≤𝐾 landings, plus a second L-Unload where the last landing is an L-Raise; from 𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀 against an 𝜔≠𝜔<sup>∗</sup> it takes an L- Divert in place of the L-Leave, and from any other state a suffix of that sequence. No further L-Divert or L-Leave falls in the interval, the 𝜔 that the L-Begin writes being target<sup>𝑡</sup> 𝑛<sup>= 𝜔∗ itself,</sup> and at 𝖠𝖼𝗍𝗂𝗏𝖾(−, 𝜔<sup>∗</sup> ), at 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(⊥) with 𝜔<sup>∗</sup> = ⊥, and at 𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(𝜉) no rule applies at all. 
...
2. _(Confluence.)_ Any two such sequences from 𝛾<sup>0</sup> taking the same orchestration steps reach ≃ ≈. 
...
|---|---|
|Γ∞|`ctx`, the frst-class context|
|𝛾∈Γ|the context tree together with everything the running system<br>has touched|
|𝔈Γ,𝔈<sup>iter</sup><br>Γ|Efect callback returning / yielding inverses|
...
|𝜋: 𝔑|`fiber.parent.fiber.uid`, the fber owning the context it was<br>instantiated on|
|derived realization (Defnition 27)|`fiber.ctx`, the child context the fber runs in|
|𝜃(Defnition 44)|`fiber.state`, the lifecycle state, whose`LOADING`is𝖱𝖾𝗅𝗈𝖺𝖽𝗂𝗇𝗀and<br>whose`FAILED`is𝖨𝗇𝖺𝖼𝗍𝗂𝗏𝖾(𝜉)|
|recover, accumulator𝑔|`fiber.dispose`, the accumulator|
|𝜔(Defnition 44)|`fiber.committed`, the committed view|
|provider𝑘(𝛾)|an`Impl`whose provider fber is`ACTIVE`|
|target(𝛾, 𝑛)|`fiber.target`, recomputed by`refresh`(Algorithm 5), where<br>⊥is`INACTIVE`|
|𝖥𝗎𝗍𝗎𝗋𝖾, inertia (Section 4.3.3)|`fiber.inertia`, the handle of the transition in fight|
|O-Insert, O-Retire (Defnition 47)|`ctx.use`and the inverse of its`callback`(Algorithm 4)|
|O-Remove|the fber dropped from its runtime, with`uid`cleared|
...
|L-Leave|`refresh`marking the fber`UNLOADING`(Line 10)|
|L-Unload|`unload`and its inertial chaining (Algorithm 5)|
|guard on L-Unload|`unload`awaiting the notifed dependents (Line 25)|
|L-Raise|the error recorded on the fber, with its`target`set to⊥|
...
**Algorithm 2** Coeffect operations 1 **function** get(ctx, key) 2 realm ← ctx[ `@@isolate` ][key] _▷_ 𝜌(𝑘) 3 **return** ctx[ `@@store` ][realm] _▷_ 𝜎(𝜌(𝑘)) 4 **function** set(ctx, key, value) 5 **function** callback() 6 realm ← ctx[ `@@isolate` ][key] _▷_ 𝜌(𝑘) 7 ctx[ `@@store` ][realm] ← value _▷_ 𝜎[𝜌(𝑘) ↦𝑣] 8 notify(ctx, [key]) 9 **return function** () 10 **delete** ctx[ `@@store` ][realm] _▷_ 𝜎∖𝜌(𝑘) 11 notify(ctx, [key]) 12 **return** ctx.effect(callback) 
...
|5|fber.target← ⊥|
|6|unload(fber)|
|7|fber←Fiber(parent: ctx, inject: component.inject)|
|8|fber.ctx←ctx[fber↦fber]|
|9|fber.apply←()↦component.apply(fber.ctx, confg)|
...
|---|
|1<br>**function**refresh(fber)|
|2<br>target← target(𝛾, 𝑛)|
|3<br>**if**target=fber.target**then** **return**|
|4<br>fber.target←target|
|5<br>**if**fber.inertia**then** **return**|
|6<br>**if**target≠ ⊥ **then**|
|7<br>fber.state← `LOADING`|
|8<br>fber.inertia←create_task(reload(fber))|
|9<br>**else**|
|10<br>fber.state← `UNLOADING` _▷ out of service before any inverse is scheduled_|
|11<br>fber.inertia←create_task(unload(fber))|
|12<br>**async function**reload(fber)|
|13<br>target0 ←fber.target|
|14<br>fber.committed←resolve(fber.inject)_▷ commit the view_|
|15<br>recover← **await**execute(fber.apply, ()↦fber.target=target0)|
|16<br>fber.dispose←recover ∘ fber.dispose|
|17<br>**if**fber.target=target0 **then**|
|18<br>fber.state← `ACTIVE`|
...
|19|notify(fber.ctx, provided(fber))|
|---|---|
|20|fber.inertia←null|
|21|**else**|
|22|fber.state← `UNLOADING`|
|23|fber.inertia←create_task(unload(fber))|
|24<br>**a**|**sync function**unload(fber)|
|25|**await**all(notify(fber.ctx, provided(fber)).map(f↦f.await()))_▷ drain dependents_|
|26|**await**fber.dispose()|
|27|fber.dispose←id|
|28|fber.committed← ⊥|
|29|**if**fber.target= ⊥ **then**|
|30|fber.state← `INACTIVE`|
|31|fber.inertia←null|
|32|**else**|
|33|fber.state← `LOADING`|
|34|fber.inertia←create_task(reload(fber))|
...
|1<br>**f**|**unction**patch_isolation(entry,𝜌<sup>′</sup>)|
|---|---|
|2<br>3|𝜌 ←entry.ctx[`@@isolate`]<br>store←entry.ctx[`@@store`]|
|4|Δ ← {𝑘| 𝜌(𝑘) ≠𝜌<sup>′</sup>(𝑘)} _▷ keys whose realm changes_|
|5|**for** 𝑘 **in** Δ **do**|
|6|entry.ctx[𝛿𝑘]←fresh tag|
|7|dif[𝑘]←(𝜌(𝑘),𝜌<sup>′</sup>(𝑘), entry.ctx[𝛿𝑘], store[𝜌(𝑘)].fber.ctx[𝛿𝑘])|
|8|entry.ctx[`@@isolate`]← 𝜌<sup>′</sup>|
|9|reload(entry.fber)|
|10|**for** 𝑘 **in** Δ **do**|
...
|11|(𝑠1,𝑠2,𝑑1,𝑑2)←dif[𝑘]|
|---|---|
|12|**if** 𝑑1 = 𝑑2 **and**store[𝑠1]**and not**store[𝑠2]**then** _▷ the binding is the entry’s own_|
|13|store[𝑠2]←store[𝑠1]|
|14|**delete**store[𝑠1]|
|15|**function**afected(fber,𝑘)|
|16|(𝑠1,𝑠2,𝑑1,𝑑2)←dif[𝑘]|
|17|**return**fber.ctx[`@@isolate`][𝑘]∈{𝑠1,𝑠2}**and**(fber.ctx[𝛿𝑘]= 𝑑1)≠(𝑑2 = 𝑑1)|
|18|notify(entry.ctx,Δ, afected)_▷ in place of the realm test of Algorithm 3_|
...
|**Algorithm 8**Module classifcation|
|---|
|1<br>**function**classify(stashed, externals)|
|2<br>accepted←stashed|
|3<br>declined←externals|
|4<br>pending← ⌀|
|5<br>**for**url**in**stashed**do**|



64 

|6<br>7|pending←pending∪(get_imports(url)∖(accepted∪declined))<br>**repeat**|
|---|---|
|8|progress←false|
...
|**Al**<br>|**gorithm 9**Stale-entrydetection<br>|
|---|---|
|1<br>|**function**get_dependencies(root, declined)<br>|
|2|deps← ⌀|
...
|5|entry.fber.dispose()|
|6|entry.fber←ctx.use(import(entry.url), entry.confg)|
|7|**catch**error|
|8|restore_caches(backup)|
|9|**for**entry**in**stale_entries**do**|
|10|entry.fber.dispose()|
|11|entry.fber←ctx.use(backup[entry.url], entry.confg)|
|12|**throw**error|
...
> 5 Koishi uses the term _plugin_ for the concept this paper formalizes as _component_ . 
